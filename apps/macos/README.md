...
- Firebase initializes at launch from a local copy of the iOS `GoogleService-Info.plist`. The status menu can **Sign in with Apple** and show the Firebase UID. The session survives quit and relaunch. Anonymous sessions are rejected. A signed-in Mac publishes a Firestore heartbeat at `/users/{uid}/devices/{deviceId}` and lists other devices (for example iPhone) under the UID. **Enable requests from my iPhone** (off by default) lets the Mac claim an allowlisted `openYouTube` command and open `https://www.youtube.com`. iOS CoCaptain Agent mode can send that same command; the Mac companion chat cannot.
